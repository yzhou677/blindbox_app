...
- **Live provider gateway (long-term production path):** Firebase Functions Mercari gateway (`functions/`) returns **provider-shaped wire JSON only** — acquisition (`MercariProviderRuntime`: `FetchMercariRuntime` today; `PlaywrightMercariRuntime` reserved), normalization, then stable `/v1/browse` DTO. Flutter must not depend on how rows are acquired. Upstream instability → gateway fixture fallback; app stays calm (no error banners). App entry: [`MarketSandboxConfig`](../lib/features/market/data/sandbox/market_sandbox_config.dart) (`MARKET_SANDBOX_MERCARI`, `MERCARI_GATEWAY_BASE_URL`) + [`MercariGatewayClient`](../lib/features/market/data/datasource/mercari/mercari_gateway_client.dart). Identity/aggregation remain app-side ([`MarketIdentityMatcher`](../lib/features/market/application/market_identity_matcher.dart), [`buildCollectibleMarketSnapshots`](../lib/features/market/application/collectible_market_aggregator.dart)). Startup: [`productionMarketSources()`](../lib/features/market/data/source/default_market_sources.dart) (asset). Refresh: [`MercariSandboxMarketSource`](../lib/features/market/data/source/mercari_sandbox_market_source.dart). See [`docs/MERCARI_GATEWAY_FUNCTIONS.md`](../docs/MERCARI_GATEWAY_FUNCTIONS.md), [`functions/src/providers/mercari/ACQUISITION.md`](../functions/src/providers/mercari/ACQUISITION.md), [`docs/MERCARI_SANDBOX.md`](../docs/MERCARI_SANDBOX.md).
