...
- **Live provider sandbox (Phase 3A, default off):** optional Mercari gateway via [`MarketSandboxConfig`](../lib/features/market/data/sandbox/market_sandbox_config.dart) (`MARKET_SANDBOX_MERCARI`, `MERCARI_GATEWAY_BASE_URL`). Gateway returns **provider-shaped wire JSON only** ([`MercariGatewayClient`](../lib/features/market/data/datasource/mercari/mercari_gateway_client.dart) — no canonical taxonomy from server). Startup stays [`productionMarketSources()`](../lib/features/market/data/source/default_market_sources.dart) (asset only). Manual pull-to-refresh on Market uses [`sandboxMarketSources()`](../lib/features/market/data/source/default_market_sources.dart) when active → merge asset session + Mercari ([`market_browse_merge.dart`](../lib/features/market/application/market_browse_merge.dart)) → identity enrich → [`MarketBrowseListingsSession`](../lib/features/market/data/market_browse_listings_session.dart). Per-provider cache: [`MarketProviderBrowseCache`](../lib/features/market/data/cache/market_provider_browse_cache.dart). Debug match summary: [`MarketMatchDiagnostics`](../lib/features/market/application/market_match_diagnostics.dart). See [`docs/MERCARI_SANDBOX.md`](../docs/MERCARI_SANDBOX.md).
