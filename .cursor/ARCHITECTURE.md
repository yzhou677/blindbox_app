...
- **Live provider sandbox (Phase 3A, default off):** optional Mercari gateway via [`MarketSandboxConfig`](../lib/features/market/data/sandbox/market_sandbox_config.dart) (`MARKET_SANDBOX_MERCARI`, `MERCARI_GATEWAY_BASE_URL`). Gateway returns **provider-shaped wire JSON only** ([`MercariGatewayClient`](../lib/features/market/data/datasource/mercari/mercari_gateway_client.dart) — retry/backoff via [`mercari_gateway_policy.dart`](../lib/features/market/data/datasource/mercari/mercari_gateway_policy.dart), pagination `cursor`/`limit`, tolerant DTO parse). Startup stays [`productionMarketSources()`](../lib/features/market/data/source/default_market_sources.dart) (asset only). Manual refresh: [`MercariSandboxMarketSource`](../lib/features/market/data/source/mercari_sandbox_market_source.dart) first page + [`installSandboxMarketBrowse`](../lib/features/market/application/market_sandbox_browse_install.dart). Calm **Load more sightings** via [`market_browse_load_more_controller.dart`](../lib/features/market/application/market_browse_load_more_controller.dart) (cap `maxMercariTotalRows`). Per-provider cache: [`MarketProviderBrowseCache`](../lib/features/market/data/cache/market_provider_browse_cache.dart) (append + disk stale). See [`docs/MERCARI_SANDBOX.md`](../docs/MERCARI_SANDBOX.md), [`docs/MARKET_PRODUCTION_HARDENING.md`](../docs/MARKET_PRODUCTION_HARDENING.md).
